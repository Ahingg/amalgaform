...
static func process(world: World, _delta: float) -> void: 
	...
	for m in machines:
		...
		var recipe := world.get_component_value(Comp.RECIPE, m)
		for key in recipe:
		...
		if world.entity_have_component(Comp.POSITION, new_entity):
			var pos := world.get_component_value(Comp.POSITION, m)
			var target_pos := world.get_component_value(Comp.POSITION, new_entity)
			target_pos["x"] += pos["x"]
			target_pos["y"] += pos["y"]
		
		var interval := world.get_component_value(Comp.INTERVAL, m)
		world.attach_component(Comp.DELAY, m, Make.delay(interval["duration"]))
