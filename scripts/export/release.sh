...
#
#   ./scripts/export/release.sh v1.1.0 "catatan singkat"
#
...
CATATAN="${2:-}"
if [ -z "$TAG" ]; then
	echo "pakai: ./scripts/export/release.sh v1.1 \"catatan singkat\"" >&2
	exit 1
fi
...
NOTARIZE=1 ./scripts/export/build_mac.sh

echo "==> bangun Windows"
OUT="$OUT" ./scripts/export/build_windows.sh
